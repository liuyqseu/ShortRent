...
public class HouseManagerImpl implements HouseManager {
	...
	public List<House> listAllUncheckedHouse(int pageSize, int pageNow) {
		...
		try {
			if (conn != null && pageSize > 0 && pageNow > 0) {
				String sql = "select * from t_house where state = 1 order by id  limit " + (pageNow * pageSize - pageSize) + ","
						+ pageSize;
	...
	public List<House> listAllcheckedHouse(int pageSize, int pageNow) {
		...
		try {
			if (conn != null && pageSize > 0 && pageNow > 0) {
				String sql = "select * from t_house where state = 2 order by id  limit " + (pageNow * pageSize - pageSize) + ","
						+ pageSize;
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
